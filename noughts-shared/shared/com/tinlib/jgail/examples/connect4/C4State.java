...
public class C4State implements State {
  ...
  static {
    p1Actions = new ArrayList<>();
    for (int i = 0; i < BOARD_WIDTH; ++i) {
      p1Actions.add(C4Action.create(PLAYER_ONE, i));
    }

    p2Actions = new ArrayList<>();
    for (int i = 0; i < BOARD_WIDTH; ++i) {
      p2Actions.add(C4Action.create(PLAYER_TWO, i));
    }
  }
  ...
  public C4State copy() {
    ...
    return new C4State(copyBoard(), new ArrayList<>(actions), currentPlayer, winner);
  }
   ...
   */
  @Override
  public C4State initializeFrom(Object state) {
    ...
    for (int row = 5; row >= 0; --row) {
      for (int column = 0; column < 7; ++column) {
        ...
        if (p == 0) {
          ...
          result.append(p == PLAYER_TWO ? "X" : "O");
        }
      }
      result.append("\n");
    }
  ...
  private List<Long> actionsForCurrentPlayer() {
    List<Long> actions = currentPlayer == PLAYER_TWO ? p2Actions : p1Actions;
