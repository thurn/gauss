...
public class TicTacToeState implements State {
  ...
  public TicTacToeState copy() {
    ...
    return new TicTacToeState(board, currentPlayer, new ArrayList<>(actions));
  }
  ...
  private List<Long> allActions() {
    ...
    int action = getCurrentPlayer() == PLAYER_ONE ? 0x0001 : 0x1000;
    int other = getCurrentPlayer() == PLAYER_ONE ? 0x1000 : 0x0001;
    for (int i = 0; i < 9; ++i) {
