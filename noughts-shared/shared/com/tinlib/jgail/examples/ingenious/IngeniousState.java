...
import com.tinlib.jgail.core.Evaluator;
...
public class IngeniousState implements State {
  ...
  public static class LowestScoreEvaluator implements Evaluator {
    ...
    public double evaluate(int player, State state) {
      ...
      List<Integer> scores = 
          new ArrayList<>(ingeniousState.scoresForPlayer(player).values());
   ...
   */
  @Override
  public IngeniousState copy() {
    if (actions == null || board == null) {
      return new IngeniousState();
    }
    return new IngeniousState(new ArrayList<>(actions), copyBoard(),
        currentPlayer, new ArrayList<>(p1Hand), new ArrayList<>(p2Hand),
        new HashMap<>(p1Score), new HashMap<>(p2Score));
  }
  ...
  public int getWinner() {
    List<Integer> p1 = new ArrayList<>(p1Score.values());
    Collections.sort(p1);
    List<Integer> p2 = new ArrayList<>(p2Score.values());
    Collections.sort(p2);
  ...
  public String actionToString(long action) {
    ...
    return "[" + IngeniousHex.toString(IngeniousPiece.getHex1(piece)) +
        IngeniousHex.toString(IngeniousPiece.getHex2(piece)) + " " +
  ...
  private Map<Integer, Integer> scoresForPlayer(int player) {
    return player == PLAYER_ONE ? p1Score : p2Score;
  }
  ...
  private List<Integer> handForPlayer(int player) {
    return player == PLAYER_ONE ? p1Hand : p2Hand;
  }
  ...
  private List<Long> allActions(List<Integer> hand) {
    List<Long> result = new ArrayList<>(300);
  ...
  private void addActionsForMove(List<Integer> hand, List<Long> list,
    ...
    for (Integer piece : hand) {
      list.add(IngeniousAction.create(piece, x1, y1, x2, y2));
    }
  }
  ...
  private List<Integer> randomHand() {
    List<Integer> hand = new ArrayList<>(HAND_SIZE);
  ...
  public String toString() {
    ...
    result.append("\nNum Actions: ").append(actions.size()).append("\n");
  ...
  private void scoresToString(StringBuilder result) {
    result.append("Scores:\n");
      ...
      for (int hex : IngeniousHex.allColors()) {
        result.append(" ").append(IngeniousHex.toString(hex)).append("=").append(p1Score.get(hex));
      }
      result.append("\n");
      result.append("Player 2:");
      for (int hex : IngeniousHex.allColors()) {
        result.append(" ").append(IngeniousHex.toString(hex)).append("=").append(p2Score.get(hex));
      }
      result.append("\n");
  }
  ...
  private void boardToString(StringBuilder result) {
    ...
    for (int y = 0; y < BOARD_SIZE; ++y) {
      ...
      for (int x = 0; x < BOARD_SIZE; ++x) {
        if (board[x][y] == 0) {
          if (x == 10) {
            result.append("[T").append(y % 10).append("]");
          } else if (y == 10) {
            result.append("[").append(x).append("T]");
          } else {
          ...
          if (board[x][y] != IngeniousHex.OFF_BOARD) {
            result.append("[").append(IngeniousHex.toString(board[x][y])).append("]");
          }
        }
      }
      result.append("\n");
  ...
  private void handsToString(StringBuilder result) {
    ...
    List<Integer> hand = handForPlayer(currentPlayer);
    for (Integer piece : hand) {
      result.append(" ").append(pieceNumber).append(") ").append(IngeniousPiece.toString(piece));
      pieceNumber++;
    }
  }
}
